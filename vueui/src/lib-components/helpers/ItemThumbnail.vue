<template>
  <div :style="cssStyle">
    <v-img
      v-if="imageSource"
      :src="imageSource"
      :max-height="size"
      :max-width="size" />
    <font-awesome-icon
      v-else
      :icon="getItemIcon()"
      :color="$root.iconColor"
      :size="iconSize"
    />
  </div>
</template>
<script>

import { watch } from 'vue'
import uitools from '../../js-modules/uitools'

const ICON_LOADING =
  'data:image/gif;base64,'
const ICON_ERROR =
  'data:image/png;base64,'

export default {

  name: "ItemThumbnail",

  props: {
    item: {
      type: [Object, String],
      required: true,
    },
    size: {
      type: Number,
      required: true,
    },
    fetchSize: {
      type: Number,
      required: false,
    },
    iconSize: {
      type: String,
      required: false,
      default: '2x',
    },
    showIconFallback: {
      type: Boolean,
      default: false,
    },
  },

  data: () => ({
    imageSource: ''
  }),

  computed: {
    cssStyle() {
      return `width: ${this.size}px; height: ${this.size}; vertical-align: center; text-align: center;`
    },
  },

  created() {
    watch(() => [this.size, this.item], () => {
      this.load()
    });

    this.load();
  },

  methods: {
    getItemIcon() {
      return uitools.getItemIcon(this.item.type);
    },
    load() {
      const _this = this

      let itemId;
      let itemHash;
      let itemUpdatedAt;

      if (typeof this.item === 'object') {

        // check if we have a thumbnail bool available. In that case omit trying to load one right away
        if (this.item.hasNoThumbnail)
          return;

        itemId = this.item._id || this.item.id;
        itemHash = this.item.hash;
        itemUpdatedAt = this.item.updatedAt;
      } else {
        itemId = this.item;
      }

      if (!itemId)
        return;

      if (!itemHash && !itemUpdatedAt) {
        console.warn("When using ItemThumbnail, make sure the item has either hash or updatedAt or cache cannot be used resulting in unnecessary many API requests!");
      }

      this.imageSource = ICON_LOADING;

      this.$store.state.nkclient
        .getThumbCache()
        .get(itemId, this.fetchSize || this.size, itemHash, itemUpdatedAt)
        .then(function (thumbnail) {
          _this.imageSource = thumbnail;
        })
        .catch(function (err) {
          if (_this.showIconFallback) {
            _this.imageSource = null
          } else {
            _this.imageSource = ICON_ERROR
          }
        });
    },
  },
}
</script>
