    ...
    if (changes && changes.length) {
        return new Audit({
            ...
            changes: changes.reduce((obj, change) => {
                const key = change.path ? change.path.join('#') : "?";

                if ((change.rhs && change.rhs.type === "Buffer") ||
                    (change.lhs && change.lhs.type === "Buffer")) {

                    // do not log buffers for their sheer size. just log the action
                    if (change.kind === 'D') {
                        obj[key] = {
                            type: types.delete
                        };
                    ...
                    if (!obj[key] && change.path.length) {
                        ...
                        if (data.from && data.from.length && data.to && data.to.length) {
                            data.type = types.edit;
                        } else if (data.from && data.from.length) {
                            data.type = types.delete;
                        } else if (data.to && data.to.length) {
                            data.type = types.add;
                        }
                        obj[key] = data;
                    }
    ...
    if (typeof changes === 'object' && changes !== null) {
        if (Object.keys(changes).filter(key => key === '_id' || key === 'id').length) {
            ...
            Object.entries(changes).forEach(([sub, value]) => {
                if (!isEmpty(value) && !(value instanceof Buffer)) {
...
const plugin = function Audit(schema) {
    ...
    schema.pre('save', function (next, options) {
        const user = options.__user;
        if (!user) {
            throw "User option not set for audit log";
        }
